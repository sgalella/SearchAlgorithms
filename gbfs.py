...
pygame.display.set_caption("Greedy best-first Search (GBFS)")
...
def greedy_best_first_search(grid, open_nodes, closed_nodes, best_path):
...
while run:
    ...
    screen.fill(BLACK)
    if is_running:
        is_running = greedy_best_first_search(grid, open_nodes, closed_nodes, best_path)
    draw_grid(grid)
    pygame.display.flip()
    clock.tick(10)
pygame.quit()
